...
async function main(): Promise<void> {
  ...
  try {
    const [oemResponse, eventResponse, guidanceResponse] = await Promise.all([
      fetch("./sl8-ep-rendezvous.oem"),
      fetch("./sl8-ep-rendezvous.events.json"),
      fetch("./sl8-ep-rendezvous.guidance.json"),
    ]);
    ...
    const updateUi = (): void => {
      ...
      byId("along-track").textContent = `${sample.along_track_error_km.toFixed(1)} km`;
      byId("altitude-error").textContent = `${sample.altitude_error_km.toFixed(2)} km`;
      byId("raan-error").textContent = `${sample.raan_error_deg.toFixed(3)} deg`;
      byId("plane-error").textContent = `${sample.plane_error_deg.toFixed(3)} deg`;
      byId("delta-v").textContent = `${sample.delta_v_m_s.toFixed(1)} m/s`;
      byId("mass").textContent = `${sample.mass_kg.toFixed(1)} kg`;
      let active = 0;
