...
// Raw earcut output is a *conforming* triangulation (adjacent triangles share whole
// edges), so there are no T-junction cracks. The flat triangles chord below the sphere,
// but the renderer's analytic horizon cull (not the depth buffer) handles globe
// occlusion, so they drape fully on the near side without being depth-culled — no
// per-triangle tessellation needed.
const LAND_LIFT_KM = 1; // just under the +3 km coast/border lines
...
function emitPolygon(rings, out) {
  ...
  for (let t = 0; t < idx.length; t += 3) {
    for (let k = 0; k < 3; k += 1) {
      const lon = flat[idx[t + k] * 2];
      const lat = flat[idx[t + k] * 2 + 1];
      const [x, y, z] = geodeticToEcef(lat, lon, LAND_LIFT_KM);
      out.push(x, y, z);
    }
  }
}
